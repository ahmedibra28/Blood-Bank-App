import { GET_TAKES, ADD_TAKE, DELETE_TAKE, UPDATE_TAKE, URL } from './types';
...
export const getTakes = () => (dispatch, getState) => {
  axios
    .get(`${URL}api/blood-taking/`, tokenConfig(getState))
    .then((takes) => {
      dispatch({
        type: GET_TAKES,
        payload: takes.data,
...
export const updateTake = (take) => (dispatch, getState) => {
  console.log(take.id);
  axios
    .put(`${URL}api/blood-taking/${take.id}/`, take, tokenConfig(getState))
    .then((res) => {
      dispatch(
        createMessage({
