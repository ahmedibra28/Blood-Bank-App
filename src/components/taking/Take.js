...
import {getInventories, updateInventory} from '../../actions/inventoryActions'
...
function Take(props) {
    ...
    const handleChange = (e) => {
      ...
      const validatePlasma = () => {
        const plasma = values.plasma && inventories.find((inv) => {
        ...
        const requestPlasma = values.plasma && requests.find((req) => {
      ...
      const validatePlatelet = () => {
        const platelet = values.platelet && inventories.find((inv) => {
        ...
        const requestPlatelet= values.platelet && requests.find((req) => {
          return req.id === parseInt(values.patient_id)
        })
      ...
      const validateRBC = () => {
        const rbc = values.rbc && inventories.find((inv) => {
        ...
        const requestRBC = values.rbc && requests.find((req) => {
          return req.id === parseInt(values.patient_id)
        })
        const newRequestRBC = {
          id: requestRBC.id,
      ...
      const validateWholeBlood = () => {
        const whole_blood = values.whole_blood && inventories.find((inv) => {
        ...
        const requestWholeBlood = values.whole_blood && requests.find((req) => {
          return req.id === parseInt(values.patient_id)
        })
      ...
      const patientIdToName = id => requests && requests.map(req => req.id === id && req.patient_id )  
    return (
